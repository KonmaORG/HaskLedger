...
runCodeGenM repPoly fixers m termScope act =
    -- Start the lambda-arg name counter far above every other Unique source.
    -- Node-id-derived let names (idToName) use the node Id as the Unique and
    -- stub names continue from the max ASG Id, so a counter starting at 0
    -- collides with them: a validator arg named with Unique 1 under a
    -- `let x_1` binder gets captured by it during alpha renaming, binding
    -- the argument to whatever x_1 holds.
    runPass cxt 50000000 act >>= \case
  ...
  where
    ...
    go vars (CodeGenContext r) =
        ...
        in CodeGenContext
            ...
            -- lambda scope. Hash-consed nodes (same Id at different lambda
            -- depths) contain Arg refs that resolve differently per scope.
            -- Keeping stale outer-scope compilations in termScope produces
            -- wrong DeBruijn terms.
    ...
    withLocalBinds toBind = letMany (M.keys toBind)
      where
        ...
        letMany (i : rest) act = do
            let nm = idToName i
            t <- compileTopDown i
            -- Nest correctly: let x1 = ... in (let x2 = ... in body).
            -- The old wrapping produced inner lets outside outer ones,
            -- creating forward references between bindings.
            local (doBind i nm) $
  ...
  where
    ...
    go dbOff nodeId = do
        withNode mempty nodeId $ \thisNode -> do
            -- A candidate found under an inner lambda (dbOff > 0) would be compiled
            -- at the bind lambda's scope with its arg indices unchanged, so every
            -- index lands dbOff levels too high (DBIndexOutOfBounds or a wrong
            -- variable). Only hoist from depth 0 -- unless the subtree contains no
            -- args at all, in which case it's position-independent. The closed
            -- check reuses safeToBind with a sentinel distance no real arg index
            -- can reach. Nodes we skip here still get bound when their own
            -- enclosing lambda runs letBinds.
            safe <- safeToBind nodeId dbOff thisNode
            closed <- safeToBind nodeId 1000000000 thisNode
            case safe && (dbOff == 0 || closed) of
